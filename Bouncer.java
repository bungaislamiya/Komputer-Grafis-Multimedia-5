...
public class Bouncer extends JPanel implements Runnable {
 ...
 double newRate;
 // Use the more 
 //stable method if a history is available.
 if (previousFilled)
    newRate = (double) numberOfFrames
        / (double) 
            (now - previousTimes[previousIndex]) * 1000.0;
 else
    newRate = 1000.0 
...
class AnimationFrame extends JFrame {
 ...
 mStatusLabel.
 setText(mFormat.format(frameRate) + " fps");
         }
